...
int main() {
    Solution s;
    vector<int> preorder({1, 3, 5, 2}), inorder({5, 3, 1, 2});
    TreeNode *t1 = buildTree(preorder, inorder);
    t1->print();
    preorder = {2, 1, 4, 3, 7}, inorder = {1, 4, 2, 3, 7};
    TreeNode *t2 = buildTree(preorder, inorder);
    t2->print();
    s.mergeTrees(t1, t2);
    t1->print();
    return 0;
